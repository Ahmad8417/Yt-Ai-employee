...
Automatically posts business content to LinkedIn to generate sales
"""
...
def main():
    ...
    parser.add_argument('action', choices=['generate', 'schedule', 'queue', 'config'],
    ...
    manager = LinkedInManager(str(vault_path))

    if args.action == 'generate':
        content = manager.generate_post_content(args.category)
        print("\n" + "="*60)
        ...
        post_file = manager.queue_post(category=args.category, approved=args.approve)
        print(f"\nPost queued: {post_file}")

    elif args.action == 'schedule':
        scheduled = manager.schedule_posts(args.days)
